...
# ================== دالة تنسيق Excel ==================
def format_excel_sheets(output, header_color="FFFF00", highlight_first_row=False):
    output.seek(0)
    ...
    for ws in wb.worksheets:
        ...
        # تلوين أول صف بعد الهيدر لو مطلوب
        if highlight_first_row and ws.title.lower() == "calls":
            first_row_fill = PatternFill("solid", fgColor="FFFF00")  # أصفر
            for cell in ws[2]:
...
def generate_vodafone_report(df):
    ...
    # ===== حفظ Excel في BytesIO مع صفحة رابعة =====
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_final.to_excel(writer, sheet_name="calls", index=False)
        imei_group.to_excel(writer, sheet_name="imei", index=False)
        site_group.to_excel(writer, sheet_name="site", index=False)
        df.to_excel(writer, sheet_name="cheet", index=False)  # <-- الصفحة الرابعة

    final_output = format_excel_sheets(output, header_color="FF0000")  # بدون تظليل أول صف
    return final_output
...
def generate_orange_report(df):
    ...
    final_output = format_excel_sheets(output, header_color="FF6600")  # بدون تظليل أول صف
    return final_output
