    ...
    def generate_etisalat_report(df):
        ...
        for col in required_cols:
            if col not in df.columns:
                ...
                return None
            # ===== الأعمدة الجديدة المطلوبة للاستثناء =====
        ...
        # ===== استثناء الصف الثاني بعد الهيدر =====
        if len(df_final) >= 2:
            second_row_index = df_final.index[1]  # الصف الثاني بعد الهيدر
            second_b_number = df_final.at[second_row_index, 'B Number']
            df_match = df[df['Originating_Number'].astype(str) == second_b_number]
            if not df_match.empty:
                df_final.at[second_row_index, 'B Full Name'] = (
                    str(df_match.iloc[0]['A_Number_Details_First_Name']) + " " + str(df_match.iloc[0]['A_Number_Details_Last_Name'])
                    )
                df_final.at[second_row_index, 'B Address'] = str(df_match.iloc[0]['ID_Num'])
                df_final.at[second_row_index, 'B_NUMBER_SITE_ADDRESS'] = str(df_match.iloc[0]['MU_Site_Address'])
                df_final.at[second_row_index, 'Latitude'] = ''
                df_final.at[second_row_index, 'Longitude'] = ''
                df_final.at[second_row_index, 'Map'] = ''
                df_final.at[second_row_index, 'SMS'] = 0
