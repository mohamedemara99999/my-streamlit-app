...
def download_file(path):
    ...
    if save_path:
        ...
        messagebox.showinfo("تم التحميل", f"تم حفظ الملف في:\n{save_path}")
...
def generate_etisalat_report():
    ...
    messagebox.showinfo("نجاح", f"تم إنشاء تقرير الاتصالات مع 3 Sheets (calls – imei – site)\nالملف:\n{output_file}")
    download_btn = tk.Button(btn_frame, text="تحميل تقرير اتصالات", command=lambda: download_file(output_file))
        ...
        if hyperlink_col:
            for row in ws.iter_rows(min_row=2, min_col=hyperlink_col, max_col=hyperlink_col):
                for cell in row:
                    if cell.value:
                        cell.font = Font(color="006400", size=12)
    format_sheet(wb["calls"])
    format_sheet(wb["imei"], hyperlink_col=3)
    format_sheet(wb["site"], hyperlink_col=3)
    wb.save(output_file)

    messagebox.showinfo("نجاح", f"تم إنشاء تقرير فودافون مع 3 Sheets (calls – imei – site)\nالملف:\n{output_file}")
    download_btn_voda = tk.Button(btn_frame, text="تحميل تقرير فودافون", command=lambda: download_file(output_file))
    download_btn_voda.pack(side="left", padx=6)
...
open_btn = tk.Button(btn_frame, text="فتح ملف Excel", command=open_excel)
open_btn.pack(side="left")

etisalat_btn = tk.Button(btn_frame, text="اتصالات", command=generate_etisalat_report)
etisalat_btn.pack(side="left", padx=6)

vodafone_btn = tk.Button(btn_frame, text="فودافون", command=generate_vodafone_report)
vodafone_btn.pack(side="left", padx=6)
